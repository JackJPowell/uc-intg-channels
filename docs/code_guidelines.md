...
> **Note:** Replace `intg-channels` with your integration folder name.

```shell
python -m pylint intg-channels
python -m flake8 intg-channels --count --show-source --statistics
python -m isort intg-channels/. --check --verbose 
python -m black intg-channels --check --verbose --line-length 120
```
...
### Format Code
```shell
python -m black intg-channels --line-length 120
```
